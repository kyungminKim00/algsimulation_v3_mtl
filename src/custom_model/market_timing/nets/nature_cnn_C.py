...
def nature_cnn(scaled_images, is_training = False, **kwargs):
    ...
    layer_3_1 = activ(conv(layer_2_1, 'c3_1', n_filters=256, filter_size=3, stride=2, init_scale=np.sqrt(2), pad='SAME',
                           stride_method='Custom', **kwargs))
    layer_3_1 = tf.compat.v1.layers.batch_normalization(
        inputs=layer_3_1, momentum=0.997, epsilon=1e-5, fused=True, training=is_training)
    ...
    layer_3_2 = activ(conv(layer_2_2, 'c3_2', n_filters=256, filter_size=3, stride=2, init_scale=np.sqrt(2), pad='SAME',
                           stride_method='Custom', **kwargs))
    layer_3_2 = tf.compat.v1.layers.batch_normalization(
        inputs=layer_3_2, momentum=0.997, epsilon=1e-5, fused=True, training=is_training)
    ...
    layer_3_3 = activ(conv(layer_2_3, 'c3_3', n_filters=256, filter_size=3, stride=2, init_scale=np.sqrt(2), pad='SAME',
                           stride_method='Custom', **kwargs))
    layer_3_3 = tf.compat.v1.layers.batch_normalization(
        inputs=layer_3_3, momentum=0.997, epsilon=1e-5, fused=True, training=is_training)
    ...
    layer_3_4 = activ(conv(layer_2_4, 'c3_4', n_filters=256, filter_size=3, stride=2, init_scale=np.sqrt(2), pad='SAME',
                           stride_method='Custom', **kwargs))
    layer_3_4 = tf.compat.v1.layers.batch_normalization(

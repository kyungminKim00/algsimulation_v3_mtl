...
from custom_model.market_timing.common import VecEnvWrapper, VecEnv, DummyVecEnv, set_global_seeds
import logger

import header.market_timing.RUNHEADER as RUNHEADER
from custom_model.market_timing.policies.policies_market_timing import LstmPolicy, get_policy_from_name, ActorCriticPolicy
...
class BaseRLModel(ABC):
    ...
    def __init__(self, policy, env, verbose=0, *, requires_vec_env, policy_base, policy_kwargs=None):
        ...
        self.observation_space_mt = None
        self.action_space_mt = None
        self.n_envs = None
        ...
        if env is not None:
            ...
            self.observation_space_mt = env.observation_space_mt
            self.action_space_mt = env.action_space_mt
            if requires_vec_env:
    ...
    def set_env(self, env):
        ...
        assert self.observation_space_mt == env.observation_space_mt, \
            "Error: the environment passed must have at least the same observation space as the model was trained on."
        assert self.action_space_mt == env.action_space_mt, \
            "Error: the environment passed must have at least the same action space as the model was trained on."

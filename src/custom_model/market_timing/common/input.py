...
def observation_input(ob_space, batch_size=None, name='Ob', scale=False):
    ...
    if isinstance(ob_space, Discrete):
        ...
        processed_observations = tf.cast(tf.one_hot(observation_ph, ob_space.n), dtype=tf.float32)
        ...
        processed_observations = tf.concat([
            tf.cast(tf.one_hot(input_split, ob_space.nvec[i]), dtype=tf.float32) for i, input_split

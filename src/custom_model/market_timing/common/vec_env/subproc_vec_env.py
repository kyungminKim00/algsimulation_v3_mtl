...
from custom_model.market_timing.common.vec_env import VecEnv, CloudpickleWrapper
from custom_model.market_timing.common.tile_images import tile_images
import header.market_timing.RUNHEADER as RUNHEADER
from util import funTime


def _worker(remote, parent_remote, env_fn_wrapper, header):
    # 2020.01.31 add code to mimic fork method
    ...
    while True:
        try:
            ...
            if cmd == 'step':
                ...
                remote.send((env.observation_space, env.action_space, env.observation_space_mt, env.action_space_mt))
            elif cmd == 'env_method':
...
class SubprocVecEnv(VecEnv):
    ...
    def __init__(self, env_fns):
        # Added
        header = env_fns[0][1]
        env_fns = [env_fn[0] for env_fn in env_fns]
        ...
        observation_space, action_space, observation_space_mt, action_space_mt = self.remotes[0].recv()

        VecEnv.__init__(self, len(env_fns), observation_space, action_space, observation_space_mt, action_space_mt)
    ...
    def set_attr_non_broadcast(self, attr_name, values, indices=None):
        ...
        :param attr_name: (str) Name of attribute to assign new value
        :param values: (obj) Value to assign to 'attr_name'
        :param indices: (list,tuple) Iterable containing indices of envs whose attr to set
        :return: (list) in case env access methods might return something, they will be returned in a list
        """

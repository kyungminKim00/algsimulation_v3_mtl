...
from custom_model.fund_selection.common.utils import linear, batch_to_seq, seq_to_batch, lstm
from custom_model.fund_selection.common.input import observation_input
from custom_model.fund_selection.policies.distributions_v1 import make_proba_dist_type, \
...
if RUNHEADER.default_net == 'nature_cnn_A':
    from custom_model.fund_selection.nets.nature_cnn_A import nature_cnn
    default_cnn = nature_cnn
elif RUNHEADER.default_net == 'nature_cnn_B':
    from custom_model.fund_selection.nets.nature_cnn_B import nature_cnn
    default_cnn = nature_cnn
elif RUNHEADER.default_net == 'inception_resnet_v2_A':
    from custom_model.fund_selection.nets.inception_resnet_v2_FS_A import inception_resnet_v2
    default_cnn = inception_resnet_v2
elif RUNHEADER.default_net == 'inception_resnet_v2_B':
    from custom_model.fund_selection.nets.inception_resnet_v2_FS_B import inception_resnet_v2
    default_cnn = inception_resnet_v2
else:
...
class LstmPolicy(ActorCriticPolicy):
    ...
    def step(self, obs, state=None, mask=None, deterministic=False, sample_th=0.8):
        ...
        if deterministic:
            ...
            action, _value, snew, neglogp = self.sess.run([self.action, self._value, self.snew, self.neglogp],
                                                          {self.obs_ph: obs, self.states_ph: state, self.masks_ph: mask,
                                                           self.sample_th_ph: sample_th})

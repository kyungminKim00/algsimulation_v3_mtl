...
def pix2pix_arg_scope():
    ...
    with tf.contrib.framework.arg_scope(
        ...
        weights_initializer=tf.compat.v1.random_normal_initializer(0, 0.02),
    ) as sc:
        return sc
...
def upsample(net, num_outputs, kernel_size, method="nn_upsample_conv"):
    ...
    if method == "nn_upsample_conv":
        net = tf.image.resize(
            net,
            [kernel_size[0] * height, kernel_size[1] * width],
...
def pix2pix_generator(
    net, num_outputs, blocks=None, upsample_method="nn_upsample_conv", is_training=False

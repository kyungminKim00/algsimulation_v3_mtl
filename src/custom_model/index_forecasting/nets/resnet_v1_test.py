...
def create_test_input(batch_size, height, width, channels):
    ...
    if None in [batch_size, height, width, channels]:
        return tf.compat.v1.placeholder(
            tf.float32, (batch_size, height, width, channels)
        )
    else:
        return tf.cast(
            np.tile(
...
class ResnetUtilsTest(tf.test.TestCase):
    ...
    def testConv2DSameEven(self):
        ...
        y1_expected = tf.cast(
            [[14, 28, 43, 26], [28, 48, 66, 37], [43, 66, 84, 46], [26, 37, 46, 22]],
            dtype=tf.float32,
        )
    ...
    def testConv2DSameOdd(self):
        ...
        y2 = resnet_utils.subsample(y1, 2)
        y2_expected = tf.cast(
            [[14, 43, 34], [43, 84, 55], [34, 55, 30]], dtype=tf.float32
        )
        y2_expected = tf.reshape(y2_expected, [1, n2, n2, 1])
    ...
    def testEndPointsV1(self):
        ...
        blocks = [
            resnet_v1.resnet_v1_block("block1", base_depth=1, num_units=2, stride=2),
            resnet_v1.resnet_v1_block("block2", base_depth=2, num_units=2, stride=1),
        ]
...
class ResnetCompleteNetworkTest(tf.test.TestCase):
    ...
    def testClassificationEndPoints(self):
        ...
        self.assertListEqual(
            end_points["predictions"].get_shape().as_list(), [2, 1, 1, num_classes]
        )
        self.assertTrue("global_pool" in end_points)
        self.assertListEqual(
            end_points["global_pool"].get_shape().as_list(), [2, 1, 1, 32]
        )
    ...
    def testEndpointNames(self):
        ...
        for block in range(1, 5):
            for unit in range(1, 4 if block < 4 else 3):
                for conv in range(1, 4):
                    expected.append(
                        "resnet/block%d/unit_%d/bottleneck_v1/conv%d"
                        % (block, unit, conv)
                    )
        ...
        expected.extend(
            ["global_pool", "resnet/logits", "resnet/spatial_squeeze", "predictions"]
        )

...
class InceptionTest(tf.test.TestCase):
    def testBuildLogits(self):
        ...
        with self.test_session():
            ...
            self.assertListEqual(
                auxlogits.get_shape().as_list(), [batch_size, num_classes]
            )
    ...
    def testBuildEndPoints(self):
        ...
        with self.test_session():
            ...
            self.assertListEqual(
                aux_logits.get_shape().as_list(), [batch_size, num_classes]
            )
            pre_pool = end_points["Conv2d_7b_1x1"]
            self.assertListEqual(
                pre_pool.get_shape().as_list(), [batch_size, 8, 8, 1536]
            )
        ...
        for index, endpoint in enumerate(endpoints):
            with tf.Graph().as_default():
                ...
                if endpoint != "PreAuxLogits":
                    self.assertTrue(
                        out_tensor.op.name.startswith("InceptionResnetV2/" + endpoint)
                    )
    ...
    def testHalfSizeImages(self):
        ...
        with self.test_session():
            ...
            pre_pool = end_points["Conv2d_7b_1x1"]
            self.assertListEqual(
                pre_pool.get_shape().as_list(), [batch_size, 3, 3, 1536]
            )
    ...
    def testGlobalPool(self):
        ...
        with self.test_session():
            ...
            pre_pool = end_points["Conv2d_7b_1x1"]
            self.assertListEqual(
                pre_pool.get_shape().as_list(), [batch_size, 8, 11, 1536]
            )
    ...
    def testGlobalPoolUnknownImageShape(self):
        ...
        with self.test_session() as sess:
            ...
            logits_out, pre_pool_out = sess.run(
                [logits, pre_pool], {inputs: images.eval()}
            )

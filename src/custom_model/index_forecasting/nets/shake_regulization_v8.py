...
def attn_layer(net):
    prob = tf.nn.softmax(
        tf.reduce_mean(
            input_tensor=tf.nn.softplus(channel_wise_fc_layer(net, "channel_wise_fc")),
            axis=[1, 2],
            keepdims=True,
        )
    )
...
def channel_wise_attn_layer(net, scope, ratio=8):
    ...
    with tf.compat.v1.variable_scope(scope, "cw_attn", [net]):
        input_reshape = slim.conv1d(
            input_reshape,
            int(n_feat_map // ratio),
            [width * height],
            scope="Conv1d_cw_attn_1x1",
            padding="VALID",
            stride=int(width * height),
            activation_fn=tf.nn.softplus,
        )
        score = tf.nn.softmax(
            tf.reduce_mean(input_tensor=input_reshape, axis=[-1], keepdims=True)
        )
...
def shakenet(scaled_images, is_training=False, **kwargs):
    ...
    with slim.arg_scope(shake_arg_scope(use_batch_norm=True)):
        with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
            ...
            net = channel_wise_attn_layer(scaled_images, f"embedding_{idx}", ratio=8)

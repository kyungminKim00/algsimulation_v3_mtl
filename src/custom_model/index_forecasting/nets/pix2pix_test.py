...
from custom_model.index_forecasting.nets import pix2pix
...
class GeneratorTest(tf.test.TestCase):
    def _reduced_default_blocks(self):
        ...
        return [
            pix2pix.Block(b.num_filters // 32, b.decoder_keep_prob)
    ...
    def test_output_size_nn_upsample_conv(self):
        ...
        with self.test_session() as session:
            ...
            self.assertListEqual(
                [batch_size, height, width, num_outputs], list(np_outputs.shape)
            )
...
class DiscriminatorTest(tf.test.TestCase):
    ...
    def test_four_layers(self):
        ...
        self.assertListEqual(
            [batch_size, output_size, output_size, 1], logits.shape.as_list()
        )
        self.assertListEqual(
            [batch_size, output_size, output_size, 1],

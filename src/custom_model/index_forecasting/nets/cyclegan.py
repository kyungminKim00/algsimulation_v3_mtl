...
def cyclegan_generator_resnet(
    ...
    paddings = np.array(
        [[0, 0], [pad_top, pad_bottom], [pad_left, pad_right], [0, 0]], dtype=np.int32
    )
            ...
            with tf.contrib.framework.arg_scope(
                ...
                for block_id in xrange(num_resnet_blocks):
                    with tf.compat.v1.variable_scope("block_{}".format(block_id)):
                        ...
                        res_net = tf.pad(
                            tensor=res_net, paddings=paddings, mode="REFLECT"
                        )
                        res_net = layers.conv2d(
                            res_net, num_filters * 4, activation_fn=None
                        )
                        net += res_net

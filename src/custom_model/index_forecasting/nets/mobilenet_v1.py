        ...
        with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding=padding):
            ...
            for i, conv_def in enumerate(conv_defs):
                ...
                if isinstance(conv_def, Conv):
                    ...
                    net = slim.conv2d(
                        net, depth(conv_def.depth), [1, 1], stride=1, scope=end_point
                    )
                    ...
                    raise ValueError(
                        "Unknown convolution type %s for layer %d" % (conv_def.ltype, i)
                    )
...
def mobilenet_v1(
    ...
    if len(input_shape) != 4:
        raise ValueError(
            "Invalid input tensor rank, expected 4, was: %d" % len(input_shape)
        )

    with tf.compat.v1.variable_scope(
        scope, "MobilenetV1", [inputs], reuse=reuse
    ) as scope:

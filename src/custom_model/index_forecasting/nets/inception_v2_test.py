...
class InceptionV2Test(tf.test.TestCase):
    def testBuildClassificationNetwork(self):
        ...
        self.assertTrue(logits.op.name.startswith("InceptionV2/Logits/SpatialSqueeze"))
        self.assertListEqual(logits.get_shape().as_list(), [batch_size, num_classes])
        self.assertTrue("Predictions" in end_points)
        self.assertListEqual(
            end_points["Predictions"].get_shape().as_list(), [batch_size, num_classes]
        )
    ...
    def testBuildAndCheckAllEndPointsUptoMixed5c(self):
        ...
        inputs = tf.random.uniform((batch_size, height, width, 3))
        _, end_points = inception.inception_v2_base(inputs, final_endpoint="Mixed_5c")
        endpoints_shapes = {
        ...
        for endpoint_name in endpoints_shapes:
            ...
            self.assertListEqual(
                end_points[endpoint_name].get_shape().as_list(), expected_shape
            )
    ...
    def testBuildEndPointsNCHWDataFormat(self):
        ...
        for key in endpoint_keys:
            transposed_original_shape = (
                tf.transpose(a=end_points[key], perm=[0, 3, 1, 2]).get_shape().as_list()
            )
            self.assertTrue(key in end_points_with_replacement)
            new_shape = end_points_with_replacement[key].get_shape().as_list()
    ...
    def testGlobalPoolUnknownImageShape(self):
        ...
        with self.test_session() as sess:
            ...
            logits, end_points = inception.inception_v2(
                inputs, num_classes, global_pool=True
            )
            self.assertTrue(logits.op.name.startswith("InceptionV2/Logits"))
            self.assertListEqual(
                logits.get_shape().as_list(), [batch_size, num_classes]
    ...
    def testBatchNormScale(self):
        ...
        num_classes = 1000
        inputs = tf.compat.v1.placeholder(tf.float32, (1, height, width, 3))
        with slim.arg_scope(inception.inception_v2_arg_scope(batch_norm_scale=True)):
            inception.inception_v2(inputs, num_classes, is_training=False)

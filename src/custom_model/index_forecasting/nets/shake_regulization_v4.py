...
def block_stem(inputs, scope=None, reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, "Stem", [inputs], reuse=reuse):
        ...
        net = slim.conv2d(
            net, 16, [1, 1], activation_fn=tf.nn.relu, scope="Conv2d_0b_1x1"
        )
        net = slim.conv2d(
            net, 21, [3, 3], activation_fn=tf.nn.relu, scope="Conv2d_0c_1x1"
        )
...
def create_residual_shortcut(net, filters, stride, is_training=False):
    ...
    net = slim.batch_norm(
        ...
        epsilon=RUNHEADER.m_batch_epsilon,
...
def channel_attention(input_feature, name, ratio=8):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        avg_pool = tf.reduce_mean(
            input_tensor=input_feature, axis=[1, 2], keepdims=True
        )
        ...
        max_pool = tf.compat.v1.layers.dense(
            inputs=max_pool,
            units=channel // ratio,
            activation=tf.nn.relu,
            name="mlp_0",
        ...
        max_pool = tf.compat.v1.layers.dense(
            inputs=max_pool, units=channel, name="mlp_1", reuse=True
        )
        assert max_pool.get_shape()[1:] == (1, 1, channel)
...
def spatial_attention(input_feature, name):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        concat = tf.compat.v1.layers.conv2d(
            concat,
            filters=1,

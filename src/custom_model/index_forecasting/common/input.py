...
def observation_input(ob_space, batch_size=None, name="Ob", scale=False):
    ...
    if isinstance(ob_space, Discrete):
        observation_ph = tf.compat.v1.placeholder(
            shape=(batch_size,), dtype=tf.int32, name=name
        )
        processed_observations = tf.cast(
            tf.one_hot(observation_ph, ob_space.n), dtype=tf.float32
        )
        return observation_ph, processed_observations

    elif isinstance(ob_space, Box):
        observation_ph = tf.compat.v1.placeholder(
            shape=(batch_size,) + ob_space.shape, dtype=ob_space.dtype, name=name
        )
        processed_observations = tf.cast(observation_ph, dtype=tf.float32)
        # rescale to [1, 0] if the bounds are defined
        if (
            scale
            and not np.any(np.isinf(ob_space.low))
            ...
            # it may cause logical errors depend on given data.(basically normalization)
        ...
        observation_ph = tf.compat.v1.placeholder(
            shape=(batch_size, ob_space.n), dtype=tf.int32, name=name
        )
        processed_observations = tf.cast(observation_ph, dtype=tf.float32)
        return observation_ph, processed_observations
        ...
        observation_ph = tf.compat.v1.placeholder(
            shape=(batch_size, len(ob_space.nvec)), dtype=tf.int32, name=name
        )
        processed_observations = tf.concat(
            [
                tf.cast(tf.one_hot(input_split, ob_space.nvec[i]), dtype=tf.float32)
        ...
        raise NotImplementedError(
            "Error: the model does not support input space of type {}".format(

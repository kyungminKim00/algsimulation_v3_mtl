...
def get_model_from_meta_repo(target_name, forward, use_historical_model=False):
    a, b, c, d = [], [], [], []
    m_name = f"./save/model_repo_meta/{target_name}_T{forward}.pkl"
    model_info = load(m_name, "pickle")
    ...
    for model in model_info:
        if not use_historical_model:
            ...
            fn = f"./save/model/rllearn/{model['m_name']}/{model['model_name']}"
...
def configure_header(args):
    ...
    for key in dict_RUNHEADER.keys():
        if key in ("_debug_on", "release", "c_epoch"):
...
def naive_filter(model_list):
    ...
    if len(filtered_model) == 0:
        print(
            "All model are denied from the naive filter. choose the last model to calculate performence"
        )
        filtered_model.append(model_list[-1])
...
class Script:
    ...
    def _inference(
        ...
        for _model in filenames:
            ...
            _model = f"{model_location}/{_model}"
            ...
            while np.sum(np.array(env.get_attr("eof"))) == 0:
                ...
                action, states, values, neglogp = self.base_model.predict(
                    obs + np.zeros([RUNHEADER.m_n_cpu] + list(obs.shape[1:])),
                    state=p_states,
                    mask=None,
                    deterministic=True,
                )
                ...
                #                                                        actions=action)
                softmax_actions = np.zeros(
                    (1, RUNHEADER.mtl_target, 2)
                )  # a dummy code for fast inference
    ...
    def learning_parameter(self, result):
        ...
        # Export to txt
        file_location = f"{result}/"
        with open(f"{file_location}agent_parameter.txt", "w") as f_out:
            for element in keys:

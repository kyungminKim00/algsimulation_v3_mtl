...
if __name__ == "__main__":
    try:
        ...
        if (
            ...
            and not (args.m_target_index is None)
            and not (args.forward_ndx is None)
        ):
            base_dirs = [
                "./save/model/rllearn",
                "./save/result",
                "./save/model/rllearn/buffer_save",
                "./save/tensorlog/index_forecasting",
                "./save/result/selected",
            ]
            ...
            for base_dir in base_dirs:
                ...
                for fn in fn_list:
                    try:
                        ...
                        if (
                            "./save/model/rllearn" in fn
                            and "./save/model/rllearn/buffer_save" not in fn
                        ):
                            if auto_clean_envs.check_model_pool(
                                fn, target_name, forward_ndx
                            ):
        ...
        # experimental mode
            ...
            for process_id in process_ids:
                try:
                    json_location = recent_procedure(
                        "./agent_log/working_model_p", process_id, "r"
                    )
                    json_location = "./save/model/rllearn/" + json_location
                    dict_RUNHEADER = util.json2dict(
                        "{}/agent_parameter.json".format(json_location)
                    )
                    # re-load
            ...
            assert (
                False
            ), "Required parameters are not given [process_id | m_target_index and forward_ndx]"

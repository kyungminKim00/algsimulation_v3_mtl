...
def splite_rawdata_v1(index_price=None, fund_tri=None):

    index_df = pd.read_csv(index_price)
    fund_df = pd.read_csv(fund_tri)

    index_dates = index_df.values[:, 0]
    index_values = np.array(index_df.values[:, 1:], dtype=np.float32)
    fund_dates = fund_df.values[:, 0]
    fund_values = np.array(fund_df.values[:, 1:], dtype=np.float32)
    ids_to_class_names = dict(zip(range(len(fund_df.keys()[1:])), fund_df.keys()[1:]))
...
def run(dataset_dir, file_pattern='fs_v0_cv%02d_%s.tfrecord'):
    ...
    fund_tri = './datasets/rawdata/ETF_index/ETF_index Price_20190623.csv'

    # Version 1: using fund raw data (csv)
    dates, sd_data, fund_data, returns, ids_to_class_names = splite_rawdata_v1(index_price=index_price,
                                                                               fund_tri=fund_tri)

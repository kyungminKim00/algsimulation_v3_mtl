...
class ReadData(object):
    ...
    def _get_returns(self, p_data, n_data):
        return ((n_data - p_data) / (np.abs(p_data)+1E-15))*100
    ...
    def _get_patch(self, base_date):
        ...
        self.status_data5 = ((self.data - previous_data) / (np.abs(previous_data) + 1E-15)) * 100
        ...
        self.class_ratio = \
            self._get_returns(self.target_data[base_date, :],
                              self.target_data[base_date + forward_ndx, :])  # +5 days
        self.class_ratio_ref0 = \
            self._get_returns(self.target_data[base_date + forward_ndx + ref_forward_ndx[1], :],
                              self.target_data[base_date, :])  # -5 days
        self.class_ratio_ref1 = \
            self._get_returns(self.target_data[base_date, :],
                              self.target_data[base_date + forward_ndx + ref_forward_ndx[1], :])  # 10
        self.class_ratio_ref2 = \
            self._get_returns(self.target_data[base_date, :],
                              self.target_data[base_date + forward_ndx + ref_forward_ndx[2], :])  # 20
        self.class_ratio_ref4 = \
            self._get_returns(self.target_data[base_date, :],
                              self.target_data[base_date + forward_ndx + ref_forward_ndx[3], :])  # 30
        self.class_ratio_ref5 = \
            self._get_returns(self.target_data[base_date, :],
                              self.target_data[base_date + forward_ndx + ref_forward_ndx[4], :])  # 40
        self.class_ratio_ref6 = \
            self._get_returns(self.target_data[base_date, :],
                              self.target_data[base_date + forward_ndx + ref_forward_ndx[5], :])  # 50
        self.class_ratio_ref7 = \
            self._get_returns(self.target_data[base_date, :],
                              self.target_data[base_date + forward_ndx + ref_forward_ndx[6], :])  # 60
        self.class_ratio_ref3 = \
            self._get_returns(self.target_data[base_date - 1, :],
                              self.target_data[base_date, :])  # 0
        ...
        if verbose == 0:  # for train and validation
            for cv_idx in range(len(index_container)):
                ...
                write_patch(sd_reader, sd_reader_ma5, sd_reader_ma10, sd_reader_ma20, sd_reader_ma60,
                            ...
                            sd_min_max_nor_reader_ma20, sd_min_max_nor_reader_ma60,
                            fund_his_reader_30, fund_cov_reader_60, extra_cov_reader_60,
                            x_seq, forward_ndx, train_list, output_filename, stride=1)
        elif verbose == 2:
...
def splite_rawdata_v1(index_price=None, y_index=None):
    ...
    ids_to_class_names = dict(zip(range(len(y_index_df.keys()[1:])), y_index_df.keys()[1:]))
    ...
    # returns
    diff_data = np.diff(y_index_values, axis=0)
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore')
        try:
            returns = (diff_data / (np.abs(y_index_values[:-1, :])+1E-15))*100
        except Warning:
            pass
    ...
    assert (len(index_dates[0]) == len(y_index_dates[0])), 'Date format are different'

    return dates, sd_data, y_index_data, returns, ids_to_class_names
...
def run(dataset_dir, file_pattern='fs_v0_cv%02d_%s.tfrecord'):
    ...
    # y_index = './datasets/rawdata/ETF_index/ETF_index Price_20190623.csv'
    y_index = './datasets/rawdata/fund_data/fund_data.csv'

    # Version 1: using fund raw data (csv)
    dates, sd_data, y_index_data, returns, ids_to_class_names = splite_rawdata_v1(index_price=index_price,
                                                                                  y_index=y_index)
    ...
    sd_diff_data_train, sd_diff_data_test = cut_off_data(sd_diff, cut_off, blind_set_seq)
    sd_velocity_data_train, sd_velocity_data_test = cut_off_data(sd_velocity, cut_off, blind_set_seq)
    sd_min_max_nor_data_train, sd_min_max_nor_data_test = cut_off_data(sd_min_max_nor, cut_off, blind_set_seq)
    ...
    sd_diff_ma_data_5_train, sd_diff_ma_data_5_test = cut_off_data(sd_diff_ma_data_5, cut_off, blind_set_seq)
    sd_diff_ma_data_10_train, sd_diff_ma_data_10_test = cut_off_data(sd_diff_ma_data_10, cut_off, blind_set_seq)
    sd_diff_ma_data_20_train, sd_diff_ma_data_20_test = cut_off_data(sd_diff_ma_data_20, cut_off, blind_set_seq)
    sd_diff_ma_data_60_train, sd_diff_ma_data_60_test = cut_off_data(sd_diff_ma_data_60, cut_off, blind_set_seq)
    ...
    extra_cor_60_train, extra_cor_60_test = cut_off_data(extra_cor_60, cut_off, blind_set_seq)
    ...
    if dependent_var == 'returns':
        ...
        target_data_train, target_data_test = cut_off_data(y_index_data, cut_off, blind_set_seq)

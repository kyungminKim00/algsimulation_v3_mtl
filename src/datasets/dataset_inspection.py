...
if __name__ == "__main__":
    ...
    filepath_train = "../save/tf_record/index_forecasting/if_x0_20_y{}_{}/if_{}_cv00_train.pkl".format(
        forward_ndx, dataset_version, dataset_version
    )
    filepath_validation = "../save/tf_record/index_forecasting/if_x0_20_y{}_{}/if_{}_cv00_validation.pkl".format(
        forward_ndx, dataset_version, dataset_version
    )
    filepath_test = "../save/tf_record/index_forecasting/if_x0_20_y{}_{}/if_{}_cv00_test.pkl".format(
        forward_ndx, dataset_version, dataset_version
    )
    meta = "../save/tf_record/index_forecasting/if_x0_20_y{}_{}/meta".format(
        forward_ndx, dataset_version
    )
    ...
    for fn in filepath_test:
        data = load(fn, "pickle")
        target_column = [
            [it["date/base_date_label"], it["date/prediction_date_label"]]
            for it in data
        ]
        string_print = target_column[-1]
        print(
            "[{}] Base: {}, Prediction: {}".format(
                fn.split("_")[-1], string_print[0], string_print[1]
            )
        )

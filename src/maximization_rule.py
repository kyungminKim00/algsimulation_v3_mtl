...
def get_data(files=None):
    ...
    for idx in range(len(data)):
        if idx == 0:
            ...
            if idx == 1:
                tot = pd.merge(
                    i_data, j_data, left_on="cnt", right_on="cnt", how="outer"
                )
            else:
        ...
        for j in classes:
            for k in classes:
                ...
                allowed_rules.append(
                    ["{}-{}-{}".format(i, j, k), [predictor, predictor_return]]
                )
...
if __name__ == "__main__":
    ...
    if args.g_combination:
        ...
        allowed_rules = search_full_combination(
            classes=[0, 1, 2], num_predictor=3, tot_refined=tot_refined
        )
        write_pickle(
            allowed_rules, "{}/{}_allowed_rules.pkl".format(root_dir, target_name)
        )
    ...
    for index, row in tot_refined.iterrows():
        ...
        r_combination = "{}-{}-{}".format(
            int(row["expected_label_x"]),
            int(row["expected_label_y"]),
            int(row["expected_label"]),
        )

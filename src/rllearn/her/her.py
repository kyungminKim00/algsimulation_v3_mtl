...
from rllearn.common import BaseRLModel, SetVerbosity
...
class HER(BaseRLModel):
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            ), "Error: HER cannot output a {} action space, only spaces.Box is supported.".format(
                self.action_space
            )
            assert not issubclass(
                self.policy, LstmPolicy
            ), "Error: cannot use a recurrent policy for the HER model."
            assert issubclass(self.policy, ActorCriticPolicy), (
                "Error: the input policy for the HER model must be an "

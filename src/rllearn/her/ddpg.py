...
def dims_to_shapes(input_dims):
    return {
        key: tuple([val]) if val > 0 else tuple() for key, val in input_dims.items()
    }
...
class DDPG(object):
    def __init__(
        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            self.buffer_ph_tf = [
                tf.compat.v1.placeholder(tf.float32, shape=shape)
        ...
        buffer_shapes = {
            key: (
                self.time_horizon if key != "o" else self.time_horizon + 1,
                *input_shapes[key],
            )
        ...
        buffer_size = (
            self.buffer_size // self.rollout_batch_size
        ) * self.rollout_batch_size
        self.buffer = ReplayBuffer(
            buffer_shapes, buffer_size, self.time_horizon, self.sample_transitions
        )

    def _random_action(self, num):
        return np.random.uniform(
            low=-self.max_u, high=self.max_u, size=(num, self.dim_action)
        )
    ...
    def get_actions(
        ...
        feed = {
            ...
            policy.u_tf: np.zeros(
                (obs.size // self.dim_obs, self.dim_action), dtype=np.float32
            ),
        }
        ...
        action = ret[0]
        noise = (
            noise_eps * self.max_u * np.random.randn(*action.shape)
        )  # gaussian noise
        action += noise
        action = np.clip(action, -self.max_u, self.max_u)
        # eps-greedy
        n_ac = action.shape[0]
        action += np.random.binomial(1, random_eps, n_ac).reshape(-1, 1) * (
            self._random_action(n_ac) - action
        )
        if action.shape[0] == 1:
            action = action[0]
        action = action.copy()
    ...
    def _vars(self, scope):
        res = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope=self.scope + "/" + scope
        )
        assert len(res) > 0
        return res
    ...
    def _global_vars(self, scope):
        res = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope=self.scope + "/" + scope
        )
        return res

    def _create_network(self, reuse=False):
        logger.info(
            "Creating a DDPG agent with action space %d x %s..."
        ...
        with tf.compat.v1.variable_scope("o_stats") as scope:
            ...
            self.o_stats = Normalizer(
                self.dim_obs, self.norm_eps, self.norm_clip, sess=self.sess
            )
        with tf.compat.v1.variable_scope("g_stats") as scope:
            if reuse:
                scope.reuse_variables()
            self.g_stats = Normalizer(
                self.dim_goal, self.norm_eps, self.norm_clip, sess=self.sess
            )
        ...
        with tf.compat.v1.variable_scope("main") as scope:
            if reuse:
                scope.reuse_variables()
            self.main = self.create_actor_critic(
                batch_tf, net_type="main", **self.__dict__
            )
            scope.reuse_variables()
...
3
        ...
        self.pi_grad_tf = flatten_grads(
            grads=pi_grads_tf, var_list=self._vars("main/pi")
        )

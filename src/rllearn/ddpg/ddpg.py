...
def get_target_updates(_vars, target_vars, tau, verbose=0):
    ...
    for var, target_var in zip(_vars, target_vars):
        ...
        soft_updates.append(
            tf.compat.v1.assign(target_var, (1.0 - tau) * target_var + tau * var)
        )
...
def get_perturbed_actor_updates(actor, perturbed_actor, param_noise_stddev, verbose=0):
    ...
    assert len(tf_util.get_globals_vars(actor)) == len(
        tf_util.get_globals_vars(perturbed_actor)
    )
    assert len(
        [
            var
            for var in tf_util.get_trainable_vars(actor)
            if "LayerNorm" not in var.name
        ]
    ) == len(
        [
            var
            for var in tf_util.get_trainable_vars(perturbed_actor)
            if "LayerNorm" not in var.name
        ]
    )
    ...
    for var, perturbed_var in zip(
        tf_util.get_globals_vars(actor), tf_util.get_globals_vars(perturbed_actor)
    ):
        if var in [
            var
            for var in tf_util.get_trainable_vars(actor)
            ...
            updates.append(
                tf.compat.v1.assign(
                    ...
                    var
                    + tf.random.normal(
                        tf.shape(input=var), mean=0.0, stddev=param_noise_stddev
                    ),
                )
            )
        else:
...
class DDPG(OffPolicyRLModel):
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            ), "Error: DDPG cannot output a {} action space, only spaces.Box is supported.".format(
                self.action_space
            )
            ...
            with self.graph.as_default():
                ...
                with tf.compat.v1.variable_scope("input", reuse=False):
                    ...
                    normalized_obs1 = tf.clip_by_value(
                        normalize(self.target_policy.processed_obs, self.obs_rms),
                        self.observation_range[0],
                        self.observation_range[1],
                    )
                    ...
                    self.param_noise_stddev = tf.compat.v1.placeholder(
                ...
                with tf.compat.v1.variable_scope("model", reuse=False):
                    ...
                    self.normalized_critic_with_actor_tf = self.policy_tf.make_critic(
                ...
                with tf.compat.v1.variable_scope("target", reuse=False):
                    critic_target = self.target_policy.make_critic(
                        normalized_obs1, self.target_policy.make_actor(normalized_obs1)
                    )
                ...
                with tf.compat.v1.variable_scope("loss", reuse=False):
                    ...
                    self.target_q = (
                        self.rewards + (1.0 - self.terminals1) * self.gamma * q_obs1
                    )

                    tf.compat.v1.summary.scalar(
                        "critic_target", tf.reduce_mean(input_tensor=self.critic_target)
                    )
                ...
                with tf.compat.v1.variable_scope("input_info", reuse=False):
                    tf.compat.v1.summary.scalar(
                        "rewards", tf.reduce_mean(input_tensor=self.rewards)
                    )
                    tf.compat.v1.summary.scalar(
                        "param_noise_stddev",
                        tf.reduce_mean(input_tensor=self.param_noise_stddev),
                    )

                    if self.full_tensorboard_log:
                        tf.compat.v1.summary.histogram("rewards", self.rewards)
                        tf.compat.v1.summary.histogram(
                            "param_noise_stddev", self.param_noise_stddev
                        )
                        if len(
                            self.observation_space.shape
                        ) == 3 and self.observation_space.shape[0] in [1, 3, 4]:
                            tf.compat.v1.summary.image("observation", self.obs_train)
                        else:
                            tf.compat.v1.summary.histogram(
                                "observation", self.obs_train
                            )
    ...
    def _setup_param_noise(self, normalized_obs0):
        ...
        with tf.compat.v1.variable_scope("noise_update_func", reuse=False):
            ...
            self.perturb_policy_ops = get_perturbed_actor_updates(
                "model/pi/", "noise/pi/", self.param_noise_stddev, verbose=self.verbose
            )
            ...
            self.adaptive_policy_distance = tf.sqrt(
                tf.reduce_mean(
                    input_tensor=tf.square(self.actor_tf - adaptive_actor_tf)
                )
            )
    ...
    def _setup_actor_optimizer(self):
        ...
        actor_shapes = [
            var.get_shape().as_list() for var in tf_util.get_trainable_vars("model/pi/")
        ]
        actor_nb_params = sum(
    ...
    def _setup_critic_optimizer(self):
        ...
        if self.critic_l2_reg > 0.0:
            ...
            if self.verbose >= 2:
                ...
                logger.info(
                    "  applying l2 regularization with {}".format(self.critic_l2_reg)
                )
        ...
        critic_shapes = [
            var.get_shape().as_list() for var in tf_util.get_trainable_vars("model/qf/")
        ]
    ...
    def _setup_popart(self):
        ...
        for out_vars in [
            ...
            [
                var
                for var in tf_util.get_trainable_vars("target/qf/")
                if "output" in var.name
            ],
        ]:
    ...
    def _setup_stats(self):
        ...
        if self.normalize_observations:
            ops += [
                tf.reduce_mean(input_tensor=self.obs_rms.mean),
                tf.reduce_mean(input_tensor=self.obs_rms.std),
            ]
    ...
    def _get_stats(self):
        ...
        feed_dict = {self.actions: self.stats_sample["actions"]}

        for placeholder in [
            self.action_train_ph,
            self.action_target,
            self.action_adapt_noise,
    ...
    def _adapt_param_noise(self):
        ...
        batch = self.memory.sample(batch_size=self.batch_size)
        self.sess.run(
            self.perturb_adaptive_policy_ops,
            feed_dict={
                self.param_noise_stddev: self.param_noise.current_stddev,
            },
        )
        distance = self.sess.run(
            self.adaptive_policy_distance,
            feed_dict={
        ...
        mean_distance = (
            MPI.COMM_WORLD.allreduce(distance, op=MPI.SUM) / MPI.COMM_WORLD.Get_size()
        )
                ...
                while True:
                    for _ in range(log_interval):
                        ...
                        for t_train in range(self.nb_train_steps):
                            ...
                            if (
                                self.memory.nb_entries >= self.batch_size
                                and t_train % self.param_noise_adaption_interval == 0
                            ):
                            ...
                            step = (
                                int(
                                    t_train
                                    * (self.nb_rollout_steps / self.nb_train_steps)
                                )
                                + self.num_timesteps
    ...
    def predict(self, observation, state=None, mask=None, deterministic=True):
        ...
        (
            actions,
            _,
        ) = self._policy(observation, apply_noise=not deterministic, compute_q=False)

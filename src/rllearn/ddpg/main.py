...
from rllearn.ddpg.noise import (
    ...
    for current_noise_type in noise_type.split(","):
        ...
        if current_noise_type == "none":
            ...
            action_noise = NormalActionNoise(
                mean=np.zeros(nb_actions), sigma=float(stddev) * np.ones(nb_actions)
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--nb-train-steps", type=int, default=50
    )  # per epoch cycle and MPI worker
    parser.add_argument(
        "--nb-eval-steps", type=int, default=100
    )  # per epoch cycle and MPI worker
    parser.add_argument(
        "--nb-rollout-steps", type=int, default=100
    )  # per epoch cycle and MPI worker

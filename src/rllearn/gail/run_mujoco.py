"""
Disclaimer: this code is highly based on trpo_mpi at @openai/rllearn and @openai/imitation
"""
...
from rllearn.gail import mlp_policy, behavior_clone
...
def argsparser():
    ...
    parser.add_argument(
        "--load_model_path", help="if provided, load the model", type=str, default=None
    )
    # Task
    parser.add_argument(
        "--task", type=str, choices=["train", "evaluate", "sample"], default="train"
    )
    # for evaluatation
    boolean_flag(
        parser,
    ...
    boolean_flag(
        parser, "save_sample", default=False, help_msg="save the trajectories or not"
    )
    ...
    parser.add_argument(
        "--save_per_iter", help="save model every xx iterations", type=int, default=100
    )
    parser.add_argument(
        "--num_timesteps", help="number of timesteps per episode", type=int, default=5e6
    )
    # Behavior Cloning
    boolean_flag(parser, "pretrained", default=False, help_msg="Use BC to pretrain")
    parser.add_argument(
        "--bc_max_iter", help="Max iteration for training BC", type=int, default=1e4
    )
...
def get_task_name(args):
    ...
    task_name = (
        ...
        + str(args.d_step)
        + ".policy_entcoeff_"
        + str(args.policy_entcoeff)
        + ".adversary_entcoeff_"
        + str(args.adversary_entcoeff)
    )
...
def main(args):
    ...
    with tf_util.make_session(num_cpu=1):
        ...
        def policy_fn(
            name, ob_space, ac_space, reuse=False, placeholders=None, sess=None
        ):
        ...
        env = bench.Monitor(
            env, logger.get_dir() and os.path.join(logger.get_dir(), "monitor.json")
        )
        env.seed(args.seed)
        ...
        if args.task == "train":
            ...
            reward_giver = TransitionClassifier(
                env, args.adversary_hidden_size, entcoeff=args.adversary_entcoeff
            )
            train(
                env,
                args.seed,
...
def train(
    ...
    if pretrained and (bc_max_iter > 0):
        ...
        pretrained_weight = behavior_clone.learn(
            env, policy_fn, dataset, max_iters=bc_max_iter
        )
...
def runner(
    ...
    for _ in tqdm(range(number_trajs)):
        traj = traj_1_generator(
            policy, env, timesteps_per_batch, stochastic=stochastic_policy
        )
        obs, acs, ep_len, ep_ret = (
            traj["ob"],
            traj["ac"],
            traj["ep_len"],
            traj["ep_ret"],

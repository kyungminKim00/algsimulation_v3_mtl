...
from rllearn.gail import run_mujoco, mlp_policy
...
def argsparser():
    ...
    parser.add_argument(
        "--env",
        type=str,
        choices=["Hopper", "Walker2d", "HalfCheetah", "Humanoid", "HumanoidStandup"],
    )
    boolean_flag(
        parser,
        "stochastic_policy",
...
def evaluate_env(env_name, seed, policy_hidden_size, stochastic, reuse, prefix):
    ...
    for i, limit in enumerate(CONFIG["traj_limitation"]):
        ...
        env.seed(seed)
        print(
            "Trajectory limitation: {}, Load checkpoint: {}, ".format(
        ...
        print(
            "Upper bound: {}, evaluation returns: {}, normalized scores: {}".format(

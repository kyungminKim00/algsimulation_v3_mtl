...
from rllearn.common import tf_util as tf_util
...
class TransitionClassifier(object):
    def __init__(self, env, hidden_size, entcoeff=0.001, scope="adversary"):
        ...
        self.build_ph()
        # Build grpah
        generator_logits = self.build_graph(
            self.generator_obs_ph, self.generator_acs_ph, reuse=False
        )
        expert_logits = self.build_graph(
            self.expert_obs_ph, self.expert_acs_ph, reuse=True
        )
        # Build accuracy
        generator_acc = tf.reduce_mean(
            input_tensor=tf.cast(
                tf.nn.sigmoid(generator_logits) < 0.5, dtype=tf.float32
            )
        )
        expert_acc = tf.reduce_mean(
            input_tensor=tf.cast(tf.nn.sigmoid(expert_logits) > 0.5, dtype=tf.float32)
        )
    ...
    def build_ph(self):
        ...
        self.generator_acs_ph = tf.compat.v1.placeholder(
            tf.float32, (None,) + self.actions_shape, name="actions_ph"
        )
        ...
        self.expert_acs_ph = tf.compat.v1.placeholder(
            tf.float32, (None,) + self.actions_shape, name="expert_actions_ph"
        )

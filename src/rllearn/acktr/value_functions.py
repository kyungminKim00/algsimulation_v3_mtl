...
class NeuralNetValueFunction(object):
    def __init__(self, ob_dim, ac_dim, verbose=1):
        ...
        obs_ph = tf.compat.v1.placeholder(
            tf.float32, shape=[None, ob_dim * 2 + ac_dim * 2 + 2]
        ...
        loss = tf.reduce_mean(input_tensor=tf.square(vpred_n - vtarg_n)) + tf.add_n(
            wd_loss
        )
        loss_sampled = tf.reduce_mean(
            input_tensor=tf.square(vpred_n - tf.stop_gradient(sample_vpred_n))
        )
        ...
        self.do_update = tf_util.function(
            [obs_ph, vtarg_n], update_op
    ...
    def _preproc(cls, path):
        ...
        return np.concatenate(
            [path["observation"], act, al_capone, np.ones((length, 1))], axis=1
        )
    ...
    def fit(self, paths, targvals):
        ...
        logger.record_tabular(
            "EVBefore", common.explained_variance(self._predict(_input), targets)
        )
        for _ in range(25):
            self.do_update(_input, targets)
        logger.record_tabular(
            "EVAfter", common.explained_variance(self._predict(_input), targets)
        )

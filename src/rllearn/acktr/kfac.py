...
from rllearn.acktr.kfac_utils import detect_min_val, factor_reshape, gmatmul
...
class KfacOptimizer:
    ...
    def get_factors(self, gradients, varlist):
        ...
        def _search_factors(gradient, graph):
            ...
            if "AddN" in bprop_op_name:
                ...
                assert (
                    len(np.unique(op_names)) == 1
                ), "Error: {} is shared among different computation OPs".format(
                    gradient.name
                )

                b_tensors = reduce(
                    lambda x, y: x + y, [_item["bpropFactors"] for _item in factors]
                )
                ...
                fprop_op_name = re.search(
                    "gradientsSampled(_[0-9]+|)/(.+?)_grad", bprop_op_name
                ).group(2)
                fprop_op = graph.get_operation_by_name(fprop_op_name)
                if fprop_op.op_def.name in KFAC_OPS:
                    # Known OPs
                    b_tensor = [
                        _i for _i in bprop_op.inputs if "gradientsSampled" in _i.name
                    ][-1]
                    ...
                    if fprop_op.op_def.name == "BiasAdd":
                        ...
                        f_tensors.append(
                            [
                                _i
                                for _i in fprop_op.inputs
                                if param.op.name not in _i.name
                            ][0]
                        )
                    ...
                    b_inputs_list = [
                        _i
                        for _i in bprop_op.inputs[0].op.inputs
                        if "gradientsSampled" in _i.name
                        if "Shape" not in _i.name
                    ]
                    if len(b_inputs_list) > 0:
                        ...
                        if b_tensor.get_shape():
                            b_tensor_shape = fprop_op.outputs[0].get_shape()
                            if (
                                len(b_tensor.get_shape()) > 0
                                and b_tensor.get_shape()[0].value is None
                            ):
                                b_tensor.set_shape(b_tensor_shape)
                            b_tensors.append(b_tensor)
            ...
            return {
                "opName": fprop_op_name,
                "op": fprop_op,
                "fpropFactors": f_tensors,
                "bpropFactors": b_tensors,
            }
            ...
            if factor_tensors[param]["opName"] == "BiasAdd":
                ...
                for item in varlist:
                    if len(factor_tensors[item]["bpropFactors"]) > 0:
                        if (
                            set(factor_tensors[item]["bpropFactors"])
                            == set(factor_tensors[param]["bpropFactors"])
            ...
            for i, param in enumerate(varlist):
                if len(factor_tensors[param][key]) > 0:
                    if (key + "_concat") not in factor_tensors[param]:
                        name_scope = factor_tensors[param][key][0].name.split(":")[0]
                        with tf.compat.v1.name_scope(name_scope):
                            factor_tensors[param][key + "_concat"] = tf.concat(
                ...
                for _, param2 in enumerate(varlist[(i + 1) :]):
                    if (len(factor_tensors[param][key]) > 0) and (
                        set(factor_tensors[param2][key])
                        == set(factor_tensors[param][key])
                    ):
                        factor_tensors[param2][key] = factor_tensors[param][key]
                        factor_tensors[param2][key + "_concat"] = factor_tensors[param][
                            key + "_concat"
                        ]
            ...
            with tf.device("/cpu"):
                ...
                for var in varlist:
                    ...
                    if op_type == "Conv2D":
                        ...
                        if (
                            operator_height == 1
                            and operator_width == 1
                            and self._channel_fac
                        ):
                ...
                for var in varlist:
                        ...
                        if fprop_factor not in tmp_stats_cache:
                            if op_type == "Conv2D":
                                ...
                                if (
                                    ...
                                    slot_fprop_factor_stats2 = tf.Variable(
                                        tf.linalg.tensor_diag(
                                            tf.ones([fprop_factor2_size])
                                        )
                                    ...
                                    # assume BHWC
                                    fprop_factor_size = (
                                        kernel_height * kernel_width * n_channels
                                    )
                            else:
                            ...
                            slot_fprop_factor_stats = tf.Variable(
                                tf.linalg.tensor_diag(tf.ones([fprop_factor_size]))
                                * self._diag_init_coeff,
                                name="KFAC_STATS/" + fprop_factor.op.name,
                                trainable=False,
                            )
                            ...
                            if op_type != "Conv2D":
                                tmp_stats_cache[fprop_factor] = self.stats[var][
                                    "fprop_concat_stats"
                                ]
                        else:
                            self.stats[var]["fprop_concat_stats"] = tmp_stats_cache[
                                fprop_factor
                            ]
                    ...
                    if bprop_factor is not None:
                        ...
                        if not (
                            (not self._blockdiag_bias)
                            and self.stats[var]["assnWeights"]
                        ):
                            if bprop_factor not in tmp_stats_cache:
                                slot_bprop_factor_stats = tf.Variable(
                                    tf.linalg.tensor_diag(
                                        tf.ones([bprop_factor.get_shape()[-1]])
                                    )
                                    * self._diag_init_coeff,
                                    name="KFAC_STATS/" + bprop_factor.op.name,
                                    trainable=False,
                                )
                                self.stats[var]["bprop_concat_stats"].append(
                                    slot_bprop_factor_stats
                                )
                                tmp_stats_cache[bprop_factor] = self.stats[var][
                                    "bprop_concat_stats"
                                ]
                            else:
                                self.stats[var]["bprop_concat_stats"] = tmp_stats_cache[
                                    bprop_factor
                                ]
    ...
    def compute_stats(self, loss_sampled, var_list=None):
        ...
        gradient_sampled = tf.gradients(
            ys=loss_sampled, xs=varlist, name="gradientsSampled"
        )
        self.gradient_sampled = gradient_sampled

        # remove unused variables
        gradient_sampled, varlist = zip(
            *[
                (grad, var)
                for (grad, var) in zip(gradient_sampled, varlist)
                if grad is not None
            ]
        )
        ...
        for var in varlist:
            ...
            for stats_var in fprop_stats_vars:
                ...
                if stats_var not in stats_updates_cache:
                        ...
                        if (
                            ...
                            if len(svd_factors) == 0:
                                ...
                                S, U, V = tf.batch_svd(
                                    tf.reshape(
                                        fprop_factor,
                                        [-1, kernel_height * kernel_width, chan],
                                    )
                                )
                            ...
                            patches = tf.image.extract_patches(
                                fprop_factor,
                                sizes=[1, convkernel_size[0], convkernel_size[1], 1],
                                strides=strides,
                                rates=[1, 1, 1, 1],
                                padding=padding,
                            )
                            ...
                            if self._approx_t2:
                                ...
                                fprop_factor = tf.reduce_mean(
                                    input_tensor=patches, axis=[1, 2]
                                )
                            else:
                                # size: (B x Oh x Ow) x C
                                fprop_factor = (
                                    tf.reshape(patches, [-1, flatten_size])
                                    / operator_height
                                    / operator_width
                                )
                    fprop_factor_size = int(fprop_factor.get_shape()[-1])
                    if (
                        stats_var_dim == (fprop_factor_size + 1)
                        and not self._blockdiag_bias
                    ):
                        if op_type == "Conv2D" and not self._approx_t2:
                            ...
                            fprop_factor = tf.concat(
                                [
                                    fprop_factor,
                                    tf.ones([tf.shape(input=fprop_factor)[0], 1])
                                    / operator_height
                                    / operator_width,
                                ],
                                1,
                    ...
                    # divided by B
                    cov = tf.matmul(
                        fprop_factor, fprop_factor, transpose_a=True
            ...
            for stats_var in bprop_stats_vars:
                if stats_var not in stats_updates_cache:
                        ...
                        if fprop_factor is not None:
                            if self._approx_t2:
                                ...
                                bprop_factor = tf.reduce_sum(
                                    input_tensor=bprop_factor, axis=[1, 2]
                                )  # T^2 terms * 1/T^2
                            else:
                                bprop_factor = (
                                    tf.reshape(bprop_factor, [-1, chan])
                                    * operator_height
                                    * operator_width
                                )  # T * 1/T terms
                        else:
                    ...
                    # way to handle this
                    bprop_factor *= tf.cast(batch_size, dtype=tf.float32)
                    ##

                    cov_b = tf.matmul(
                        bprop_factor, bprop_factor, transpose_a=True
                    ) / tf.cast(tf.shape(input=bprop_factor)[0], dtype=tf.float32)
        ...
        if KFAC_DEBUG:
            ...
            stats_updates[a_key] = tf.compat.v1.Print(
                stats_updates[a_key],
                [
    ...
    def apply_stats(self, stats_updates):
        ...
        def _update_accum_stats():
            if self._full_stats_init:
                return tf.cond(
                    pred=tf.greater(self.sgd_step, self._cold_iter),
                    true_fn=lambda: tf.group(
                        *self._apply_stats(
                            stats_updates,
                            accumulate=True,
                            accumulate_coeff=1.0 / self._stats_accum_iter,
                        )
                    ),
                    false_fn=tf.no_op,
                )
        ...
        if self._async_stats:
            ...
            update_stats_op = tf.cond(
                pred=tf.equal(queue.size(), tf.convert_to_tensor(value=0)),
                true_fn=tf.no_op,
            ...
            update_stats_op = tf.cond(
                pred=tf.greater_equal(self.stats_step, self._stats_accum_iter),
                true_fn=lambda: _update_running_avg_stats(stats_updates),
                false_fn=_update_accum_stats,
            )
    ...
    def _apply_stats(self, stats_updates, accumulate=False, accumulate_coeff=0.0):
        ...
        for stats_var in stats_updates:
            ...
            if accumulate:
                ...
                update_op = tf.compat.v1.assign_add(
                    update_op, (1.0 - self._stats_decay) * stats_new, use_locking=True
                )
        ...
        if KFAC_DEBUG:
            stats_step_op = tf.compat.v1.Print(
                stats_step_op,
                [
                    tf.convert_to_tensor(value="step:"),
                    self.global_step,
                    ...
                    for key in ["fprop_concat_stats", "bprop_concat_stats"]:
                        for stats_var in stats[var][key]:
                            if stats_var not in tmp_eigen_cache:
                                ...
                                eigen_vectors = tf.Variable(
                                    tf.linalg.tensor_diag(tf.ones([stats_dim])),
                                    name="KFAC_FAC/"
                                    + stats_var.name.split(":")[0]
                                    + "/Q",
                                    trainable=False,
                                )
                                stats_eigen[stats_var] = {
                                    "e": eigen_values,
                                    "Q": eigen_vectors,
                                }
            ...
            with tf.control_dependencies([]):
                for stats_var in stats_eigen:
                    if stats_var not in computed_eigen:
                        ...
                        computed_eigen[stats_var] = {
                            "e": eigen_values,
                            "Q": eigen_vectors,
                        }
                        eigen_reverse_lookup[eigen_values] = stats_eigen[stats_var]["e"]
                        eigen_reverse_lookup[eigen_vectors] = stats_eigen[stats_var][
                            "Q"
                        ]
            ...
            if KFAC_DEBUG:
                ...
                with tf.control_dependencies(update_ops):
                    update_ops.append(
                        tf.compat.v1.Print(
                            tf.constant(0.0),
                            [tf.convert_to_tensor(value="computed factor eigen")],
                        )
                    )
        ...
        with tf.control_dependencies(update_ops):
            ...
            if KFAC_DEBUG:
                update_ops.append(
                    tf.compat.v1.Print(
                        tf.constant(0.0),
                        [tf.convert_to_tensor(value="updated kfac factors")],
                    )
                )
        return update_ops
    ...
    def get_kfac_precond_updates(self, gradlist, varlist):
            ...
            if (len(fprop_factored_fishers) + len(bprop_factored_fishers)) > 0:
                ...
                if len(grad.get_shape()) > 2:
                    ...
                    if len(fprop_factored_fishers) > 1 and self._channel_fac:
                        ...
                        grad = tf.reshape(
                            grad, [kernel_width * kernel_height, n_channels, depth]
                        )
                    else:
                ...
                if (
                    self.stats[var]["assnBias"] is not None
                ) and not self._blockdiag_bias:
                ...
                for idx, stats in enumerate(self.stats[var]["fprop_concat_stats"]):
                    ...
                    eigen_values = detect_min_val(
                        self.stats_eigen[stats]["e"], var, name="act", debug=KFAC_DEBUG
                    )

                    eigen_vectors, eigen_values = factor_reshape(
                        eigen_vectors, eigen_values, grad, fac_idx=idx, f_type="act"
                    )
                    eig_vals.append(eigen_values)
                    grad = gmatmul(
                        eigen_vectors, grad, transpose_a=True, reduce_dim=idx
                    )

                for idx, stats in enumerate(self.stats[var]["bprop_concat_stats"]):
                    eigen_vectors = self.stats_eigen[stats]["Q"]
                    eigen_values = detect_min_val(
                        self.stats_eigen[stats]["e"], var, name="grad", debug=KFAC_DEBUG
                    )

                    eigen_vectors, eigen_values = factor_reshape(
                        eigen_vectors, eigen_values, grad, fac_idx=idx, f_type="grad"
                    )
                    eig_vals.append(eigen_values)
                    grad = gmatmul(
                        grad, eigen_vectors, transpose_b=False, reduce_dim=idx
                    )
                ...
                if var in self._weight_decay_dict:
                    ...
                    if KFAC_DEBUG:
                        print(
                            (
                                "weight decay coeff for %s is %f"
                                % (var.name, weight_decay_coeff)
                            )
                        )
                ...
                if self._factored_damping:
                    ...
                    if len(eig_vals) == 1:
                        ...
                        damping = tf.pow(
                            self._epsilon + weight_decay_coeff, 1.0 / num_factors
                        )
                    eig_vals_tnorm_avg = [
                        tf.reduce_mean(input_tensor=tf.abs(e)) for e in eig_vals
                    ]
                    for eigen_val, e_tnorm in zip(eig_vals, eig_vals_tnorm_avg):
                        ...
                        if len(eig_vals) == 1:
                            ...
                            adjustment = tf.pow(
                                tf.pow(e_tnorm, num_factors - 1.0)
                                / eig_tnorm_neg_list_prod,
                                1.0 / num_factors,
                            )
                ...
                for idx, stats in enumerate(self.stats[var]["fprop_concat_stats"]):
                    eigen_vectors = self.stats_eigen[stats]["Q"]
                    grad = gmatmul(
                        eigen_vectors, grad, transpose_a=False, reduce_dim=idx
                    )

                for idx, stats in enumerate(self.stats[var]["bprop_concat_stats"]):
                    eigen_vectors = self.stats_eigen[stats]["Q"]
                    grad = gmatmul(
                        grad, eigen_vectors, transpose_b=True, reduce_dim=idx
                    )
                ...
                if (
                    ...
                    grad_assn_bias = tf.reshape(
                        tf.slice(grad, begin=[c_plus_one - 1, 0], size=[1, -1]),
                        var_assn_bias.get_shape(),
                    )
                    grad_assn_weights = tf.slice(
                        grad, begin=[0, 0], size=[c_plus_one - 1, -1]
                    )
        ...
        for grad_1, var in zip(gradlist, varlist):
            ...
            if KFAC_DEBUG:
                ...
                tf.compat.v1.Print(
                    grad,
                    [tf.sqrt(tf.reduce_sum(input_tensor=tf.pow(grad, 2)))],
        ...
        if KFAC_DEBUG:
            scaling = tf.compat.v1.Print(
                scaling,
                [
                    tf.convert_to_tensor(value="clip: "),
                    scaling,
                    tf.convert_to_tensor(value=" vFv: "),
                    v_g,
                ],
            )
    ...
    def apply_gradients_kfac(self, grads):
        ...
        if self._async_eigen_decomp:
            ...
            queue = tf.queue.FIFOQueue(
                1,
                [item.dtype for item in factor_ops_dummy],
                shapes=[item.get_shape() for item in factor_ops_dummy],
            )
            enqueue_op = tf.cond(
                pred=tf.logical_and(
                    tf.equal(
                        tf.math.floormod(self.stats_step, self._kfac_update),
                        tf.convert_to_tensor(value=0),
                    ),
                    tf.greater_equal(self.stats_step, self._stats_accum_iter),
                ),
                true_fn=lambda: queue.enqueue(self.compute_stats_eigen()),
                false_fn=tf.no_op,
            )
            ...
            with tf.control_dependencies(dependency_list):
                ...
                if not self._async_eigen_decomp:
                    ...
                    update_factor_ops = tf.cond(
                        ...
                        true_fn=lambda: tf.group(
                            *self.apply_stats_eigen(self.compute_stats_eigen())
                        ),
                ...
                with tf.control_dependencies([update_factor_ops]):
                    ...
                    u = tf.cond(
                        pred=tf.greater(
                            self.factor_step, tf.convert_to_tensor(value=0)
                        ),
                        true_fn=get_kfac_grad_op,
                        false_fn=grad_op,
                    )

                    optim = tf.compat.v1.train.MomentumOptimizer(
                        self._lr * (1.0 - self._momentum), self._momentum
                    )
                        ...
                        def update_optim_op():
                            if self._full_stats_init:
                                return tf.cond(
                                    ...
                                    true_fn=lambda: optim.apply_gradients(
                                        list(zip(u, varlist))
                                    ),
                                    false_fn=tf.no_op,
                                )
                        ...
                        if self._full_stats_init:
                            return tf.cond(
                                pred=tf.greater_equal(
                                    self.stats_step, self._stats_accum_iter
                                ),
                                true_fn=update_optim_op,
                                false_fn=tf.no_op,
                            )
                        else:
                            return tf.cond(
                                pred=tf.greater_equal(self.sgd_step, self._cold_iter),
                                true_fn=update_optim_op,
                                false_fn=tf.no_op,
                            )
    ...
    def apply_gradients(self, grads):
            ...
            if KFAC_DEBUG:
                with tf.control_dependencies([sgd_step_op, cold_optim_op]):
                    sgd_step_op = tf.compat.v1.Print(
                        ...
                        [
                            self.sgd_step,
                            tf.convert_to_tensor(value="doing cold sgd step"),
                        ],
                    )
        ...
        return (
            tf.cond(
                pred=tf.greater(self.sgd_step, self._cold_iter),
                true_fn=_warm_kfac_start,
                false_fn=_cold_sgd_start,
            ),
            queue_runner,
        )
